...
const LoginJobseeker = ({ navigation, signIn, notifications, name, route }) => {
  ...
  const navigateNextFreelancer = () => {
    if (route.params.edit) {
      ...
      navigation.dispatch(StackActions.replace("seeker_dash"));
    }
  };
      ...
      dispatch(
        ...
        .then((res) => {
          ...
          if (res.user.role === "freelancer") {
            dispatch(getFreelancer(res.user.freelancerId))
              .then((res) => {
                console.log("res Freelancer:", res);
                setLoading(false);
            ...
            dispatch(getClientbyId(res.user.clientId))
              .then((res) => {
                setLoading(false);
                navigateNextClient();
              })
  ...
  return loading ? (
    ...
    <View style={styles.container}>
      <Header
        icon={settingsIcon}
        title={route.params.edit ? "Log in to Edit" : "Log in"}
        goBack={goBack}
      />
